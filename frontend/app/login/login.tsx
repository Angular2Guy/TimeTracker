...
export function Login() {
  ...
  const handleChangePassword1: ChangeEventHandler<HTMLInputElement> = (
    event,
  ) => {
    setPassword1(event.currentTarget.value as string);
  };
  const handleChangePassword2: ChangeEventHandler<HTMLInputElement> = (
    event,
  ) => {
  ...
  async function handleSubmit(
    ...
    const response =
      activeTab === 0
        ? await postLogin(email, password1, controller.current)
        : await postSignin(email, username, password1, controller.current);
